...
Its always good to have some conventions in place for file system heirarchy. For my setup, I usually have a top level `/projects` folder that contains a `playground` (for experiments) and a `stable` folder for things I intend to maintain. Since what we're doing here is more of an exercise or experiment, the prefix for everything I'll be doing is `/projects/playground/minsys`. Of course you can change this to whatever you want, just keep the substituion in mind as you follow along.
