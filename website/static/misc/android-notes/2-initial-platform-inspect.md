...
So far, we've been looking at studying the over functionality and attributes of an installed or target application. That application, for our purposes, will be running on a platform of some kind. We want to understand that platform as much as reasonable. For example, locating and writing down the following hardware information is key:
...
- **Bootloader Unlock** - When in Fastboot Mode, a user can (roughly) run `fastboot oem unlock` to unlock the bootloader. There are challenge questions that must be answered to allow this to occur. But when successful, the Bootloader Unlock skips signature verification of kernels and system images. This allows a complete replacement of the vendor provided Android with something else, or a patched version of the vendor provided system.

- **Boot.img Rooting** - When booting Android, a `boot.img` is first booted. This contains the kernel code and the initramfs. Magisk is a common tool used to enable root access to the Android system. Magisk gains root access itself by embedding itself in the initramfs of the `boot.imt`. By running from the initramfs, magisk is started with root access before the main Android system image is ever loaded.

### The Questions

Some good questions to assist with researching rooting an Android device:

- Is there a burn down for bootloader unlocking? What other contraints do we need to meet for OEM unlocking.
- Can device bootloader be unlocked? If yes, what is the expected procedure?
- Is there a vendor registration for unlocking the bootloader?
- What capabilities support my device? TWRP? Magisk?
...
- How long until system images are unlockable from past versions. For example, maybe my new "Device version X" phone isn't unlockable, but 3 versions back "Device version X-3" became unlockable a year after release.

### The Process

Please keep in mind that the process or procedure to root a device can change from device to device (event with the same vendor or make). Also, while I am writing instructions for the process here, these are not instructions to be followed. Do not unlock your bootloader or root Android without performing the proper research and developing your own procedure. 
...
- When you are in Fastboot Mode, you may have arbitrary access to read and write to flash. The system will let you shoot yourself in the foot by loading anything onto the flash. It doesn't actually enforce or check that your patches are good and correct until you reboot.
  ...
  - Don't event attempt to patch boot.img until you are confident you'd unlocked the bootloader.
